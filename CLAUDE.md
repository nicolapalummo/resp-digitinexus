...
- **Registro**: `content/blog/index.ts` importa tutti gli articoli (75 IT + 75 EN) ed esporta `ARTICLES` (usato da `lib/blog.ts` via `import ... from '../content/blog'`). Cluster: `c1`-`c8` in `content/blog/config.ts` (c8 = Ads & Funnel, uscita **domenicale** 07/06→13/09/2026 in parallelo a lun/mer/ven; brief in `docs/DigitiNexus_Cluster_Ads_Funnel.md`).
...
- `lib/blogRouting.ts` è una versione **leggera** (senza import di ARTICLES) usata dai componenti eager (Header, ExploreGuides) per non trascinare i ~150 moduli articolo nel bundle principale → blog in chunk separato (lazy).
- **Cover**: `scripts/generate-covers.mjs` (OpenAI Images, richiede `OPENAI_API_KEY`) + `scripts/generate-covers-en.mjs` (overlay titoli EN, no API; deriva il raw togliendo il suffisso `-en` dal cover.src e salta le EN già esistenti). Dati in `content/blog/covers.ts`. ⚠️ Gli script leggono **`.env.local` con precedenza su `.env`**: se l'API dà 401 con una chiave valida, controlla che non ci sia una chiave vecchia in `.env.local`.
...
- **Recensioni**: 9 testimonianze in i18n (`reviews.review1`-`review9`: quote/highlight/author, **niente ruolo**). Le foto profilo stanno in `public/avatar/` con **nomi neutri** (`avatar-N.webp`), alt vuoto e **non devono finire su Google Images**: `X-Robots-Tag: noindex, noimageindex` su `/avatar/*` (vercel.json) + `Disallow /avatar/` per Googlebot-Image (robots.txt, generato da prerender-blog). Non usare nomi di persona nei file, non rimuovere le protezioni. `highlight` deve essere una substring ESATTA della quote.
